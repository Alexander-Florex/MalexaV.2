    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="card table-card">
          <table>
            <thead><tr><th>Fecha</th><th>Categoria</th><th>Descripcion</th><th>Medio</th><th>Monto</th><th></th></tr></thead>
            <tbody>
              {expenses.map((e, i) => (
                <motion.tr key={e.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: Math.min(i * 0.02, 0.3) }}>
                  ...
                  <td data-label="Medio">{PAYMENT_METHOD_LABELS[e.payment_method] ?? '—'}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nuevo gasto">
        ...
        <form className="form-grid" onSubmit={handleSubmit}>
          ...
          <div className="field">
            ...
            <select className="select-input" value={form.paymentMethod} onChange={(e) => setForm({ ...form, paymentMethod: e.target.value as PaymentMethod })}>
              {(Object.keys(PAYMENT_METHOD_LABELS) as PaymentMethod[]).map((pm) => (
                <option key={pm} value={pm}>{PAYMENT_METHOD_LABELS[pm]}</option>
              ))}
            </select>
            <p className="field-hint">Si no es efectivo, no afecta el conteo de caja física.</p>
          </div>
